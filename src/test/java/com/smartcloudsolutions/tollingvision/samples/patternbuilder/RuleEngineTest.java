...
/**
 * Comprehensive unit tests for RuleEngine class.
 * Tests role classification accuracy, precedence handling, regex generation,
 * and error handling for various rule configurations.
 */
class RuleEngineTest {
    ...
    class FilenameClassificationTests {
        ...
        void shouldHandleCaseSensitivity() {
            ...
            assertEquals(ImageRole.FRONT, ruleEngine.classifyFilename("vehicle_FRONT_001.jpg", rules));
            assertNull(ruleEngine.classifyFilename("vehicle_front_001.jpg", rules)); // Should not match case-sensitive rule
    ...
    class PrecedenceTests {
        ...
        void shouldApplyOverviewPrecedence() {
            List<RoleRule> rules = Arrays.asList(
                ...
                new RoleRule(ImageRole.FRONT, RuleType.CONTAINS, "view", false, 2),  // Would match "overview" too
                new RoleRule(ImageRole.REAR, RuleType.CONTAINS, "over", false, 3)    // Would match "overview" too
            );
            
            // Overview should take precedence even though other rules would also match
            assertEquals(ImageRole.OVERVIEW, ruleEngine.classifyFilename("vehicle_overview_001.jpg", rules));
            assertEquals(ImageRole.FRONT, ruleEngine.classifyFilename("vehicle_frontview_001.jpg", rules));
        }
        ...
        void shouldProcessRolesInPrecedenceOrder() {
            List<RoleRule> rules = Arrays.asList(
                new RoleRule(ImageRole.REAR, RuleType.CONTAINS, "cam", false, 1),     // Would match first alphabetically
                new RoleRule(ImageRole.FRONT, RuleType.CONTAINS, "cam", false, 2),    // Would match first alphabetically  
                new RoleRule(ImageRole.OVERVIEW, RuleType.CONTAINS, "cam", false, 3)  // Should match due to precedence
            );
        ...
        void shouldRespectRulePriorityWithinRole() {
            List<RoleRule> rules = Arrays.asList(
                new RoleRule(ImageRole.FRONT, RuleType.CONTAINS, "specific", false, 1),  // Higher priority
    ...
    class RegexPatternGenerationTests {
        ...
        void shouldGenerateRegexForStartsWithRule() {
            List<RoleRule> rules = Arrays.asList(
                new RoleRule(ImageRole.FRONT, RuleType.STARTS_WITH, "front", false, 1)
            );
        ...
        void shouldGenerateRegexForEndsWithRule() {
            List<RoleRule> rules = Arrays.asList(
                new RoleRule(ImageRole.FRONT, RuleType.ENDS_WITH, "front", false, 1)
            );
        ...
        void shouldGenerateCaseSensitiveRegex() {
            List<RoleRule> rules = Arrays.asList(
                new RoleRule(ImageRole.FRONT, RuleType.CONTAINS, "FRONT", true, 1)
            );
    ...
    class ErrorHandlingTests {
        ...
        void shouldThrowExceptionForNullFilename() {
            ...
            assertThrows(IllegalArgumentException.class, 
                () -> ruleEngine.classifyFilename(null, rules));
        }
        ...
        void shouldThrowExceptionForEmptyFilename() {
            ...
            assertThrows(IllegalArgumentException.class, 
                () -> ruleEngine.classifyFilename("", rules));
            assertThrows(IllegalArgumentException.class, 
                () -> ruleEngine.classifyFilename("   ", rules));
        }
        ...
        void shouldThrowExceptionForNullRules() {
            assertThrows(IllegalArgumentException.class, 
                () -> ruleEngine.classifyFilename("test.jpg", null));
        }
        ...
        void shouldHandleInvalidRegexPattern() {
            ...
            assertThrows(IllegalArgumentException.class, 
                () -> ruleEngine.classifyFilename("test.jpg", rules));
        }
    ...
    class ValidationTests {
        ...
        void shouldDetectEmptyRuleValues() {
            ...
            boolean hasEmptyRuleWarning = result.getWarnings().stream()
        ...
        void shouldHandleNullRulesList() {
            ...
            assertFalse(result.isValid());
            assertEquals(1, result.getErrors().size());
            assertEquals(ValidationErrorType.INVALID_RULE_CONFIGURATION, result.getErrors().get(0).getType());
        }
    }
    ...
    class EdgeCasesTests {
        ...
        void shouldHandleComplexPrecedenceScenarios() {
            List<RoleRule> rules = Arrays.asList(
                ...
                new RoleRule(ImageRole.FRONT, RuleType.CONTAINS, "front", false, 1),
                new RoleRule(ImageRole.FRONT, RuleType.CONTAINS, "scene_front", false, 2), // Would match "scene" too
            ...
            assertEquals(ImageRole.OVERVIEW, ruleEngine.classifyFilename("vehicle_scene_front_001.jpg", rules));
        }
    }
}