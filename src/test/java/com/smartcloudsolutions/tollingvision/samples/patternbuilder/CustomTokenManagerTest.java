...
class CustomTokenManagerTest {
    ...
    void testAddCustomToken() {
        CustomTokenManager.CustomToken token = new CustomTokenManager.CustomToken(
    ...
    void testEnhanceWithCustomTokens() {
        // Add a custom token
        CustomTokenManager.CustomToken laneToken = new CustomTokenManager.CustomToken(
            "Lane", "Traffic lane identifier", Set.of("lane1", "lane2"), TokenType.SUFFIX
        );
        customTokenManager.addCustomToken(laneToken);
        
        // Create test tokens
        List<FilenameToken> tokens = List.of(
        ...
        TokenAnalysis originalAnalysis = new TokenAnalysis(
            List.of("test.jpg"), tokenizedFilenames, List.of(), Map.of()
        );
        ...
        FilenameToken enhancedLaneToken = enhancedTokens.stream()
            .filter(t -> t.getValue().equals("lane1"))
            .findFirst()
            .orElse(null);
