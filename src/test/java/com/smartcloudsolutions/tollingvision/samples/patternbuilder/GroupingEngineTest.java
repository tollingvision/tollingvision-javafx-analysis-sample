...
class GroupingEngineTest {
    ...
    void testBasicGrouping() {
        List<String> filenames = List.of(
            ...
            "vehicle_ABC123_rear.jpg",
            "vehicle_XYZ789_front.jpg"
        );
        
        String groupPattern = "^vehicle_([A-Z0-9]+)_\\w+\\.jpg$";
        
        List<RoleRule> roleRules = List.of(
    ...
    void testComplexFilenameWithUnknownSegment() {
        List<String> filenames = List.of(
            "52_08354353_15864_lpr_rear.jpg",
            "52_08354353_15864_lpr_front.jpg"
        );
    ...
    void testOverviewPrecedence() {
        ...
        List<RoleRule> roleRules = List.of(
            new RoleRule(ImageRole.OVERVIEW, RuleType.CONTAINS, "overview", false, 0), // Highest priority
            new RoleRule(ImageRole.FRONT, RuleType.CONTAINS, "front", false, 1),
            new RoleRule(ImageRole.REAR, RuleType.CONTAINS, "rear", false, 2)
        );
    ...
    void testNoMatchingFiles() {
        ...
        assertEquals(0, result.getMatchedFiles());
        assertEquals(1, result.getUnmatchedFiles().size());
        assertEquals("Filename doesn't match group pattern", result.getUnmatchedReasons().get("nomatch.jpg"));
    }
    
    @Test
    void testValidateGroupPattern() {
        // Valid pattern with one capturing group
        ValidationResult result = groupingEngine.validateGroupPattern("^vehicle_([A-Z0-9]+)_\\w+\\.jpg$");
        ...
        assertFalse(result.isValid());
        assertEquals(ValidationErrorType.MULTIPLE_CAPTURING_GROUPS, result.getErrors().get(0).getType());
