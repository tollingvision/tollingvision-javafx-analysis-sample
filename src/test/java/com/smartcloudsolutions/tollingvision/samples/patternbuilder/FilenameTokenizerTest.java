...
/**
 * Comprehensive unit tests for FilenameTokenizer covering various filename formats
...
class FilenameTokenizerTest {
    ...
    void testAnalyzeStandardVehicleFilenames() {
        ...
        assertTrue(analysis.getSuggestions().stream()
            .anyMatch(s -> s.getType() == TokenType.CAMERA_SIDE));
        
        // Should detect extension tokens
        assertTrue(analysis.getSuggestions().stream()
            .anyMatch(s -> s.getType() == TokenType.EXTENSION));
        
        // Should detect index tokens
        assertTrue(analysis.getSuggestions().stream()
            .anyMatch(s -> s.getType() == TokenType.INDEX));
    }
    ...
    void testGroupIdDetection() {
        ...
        assertTrue(analysis.getSuggestions().stream()
            .anyMatch(s -> s.getType() == TokenType.GROUP_ID));
    }
    ...
    void testPrefixDetection() {
        ...
        assertTrue(analysis.getSuggestions().stream()
            .anyMatch(s -> s.getType() == TokenType.PREFIX));
    }
    ...
    void testSuggestTokenTypesWithConfidence() {
        ...
        Map<String, List<FilenameToken>> tokenized = Map.of(
