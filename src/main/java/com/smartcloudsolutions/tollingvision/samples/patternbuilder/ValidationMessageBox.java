 ...
 * Automatically hides itself when there are no errors or warnings to show,
 * ensuring no empty containers are displayed in the UI.
 */
public class ValidationMessageBox extends VBox {
    ...
    private void setupBindings() {
        ...
        validationModel.hasWarningsProperty().addListener((obs, oldVal, newVal) -> updateSummaryStyle());
        ...
        validationModel.getErrors().addListener((javafx.collections.ListChangeListener<ValidationError>) c -> {
            updateErrorContent();
        });

        // Update warning content when warnings change
        validationModel.getWarnings().addListener((javafx.collections.ListChangeListener<ValidationWarning>) c -> {
    ...
    private void updateSummaryStyle() {
        summaryLabel.getStyleClass().removeAll("validation-success", "validation-warning", "validation-error");
