 ...
 * Provides debounced validation refresh, reactive updates, and consistent
 * validation state management across all pattern builder components.
 */
public class ValidationModel {
    ...
    private void setupBindings() {
        ...
        validationResult.addListener((obs, oldResult, newResult) -> {
            if (newResult != null) {
                Platform.runLater(() -> {
        ...
        currentConfiguration.addListener((obs, oldConfig, newConfig) -> {
    ...
    private void updateValidationSummary(ValidationResult result) {
        if (result.hasErrors()) {
            if (result.getErrors().size() == 1) {
                ...
                validationSummary.set(String.format("✗ " + messages.getString("status.validation.errors"), result.getErrors().size()));
            }
        } else if (result.hasWarnings()) {
            if (result.getWarnings().size() == 1) {
                validationSummary.set("⚠ " + result.getWarnings().get(0).getMessage());
            } else {
                validationSummary.set(String.format("⚠ " + messages.getString("status.configuration.valid.warnings"), result.getWarnings().size()));
            }
        } else {
    ...
    /**
     * Requests a debounced validation refresh.
     * Multiple calls within the debounce period will be coalesced into a single
     * validation.
     */
    ...
    /**
     * Immediately performs validation without debouncing.
     * Use this for critical validation updates that need immediate feedback.
     */
    ...
    private void performValidation() {
        ...
        if (config == null) {
            updateValidationState(ValidationResult.failure(
                    ValidationError.of(ValidationErrorType.INVALID_RULE_CONFIGURATION,
     ...
     */
    private ValidationResult enhanceValidationWithContext(ValidationResult baseResult, PatternConfiguration config) {
        ...
        if (sampleFilenames.isEmpty()) {
            additionalWarnings.add(ValidationWarning.of(
            ...
            try {
                ...
                if (result.getMatchedFiles() == 0) {
                    ...
                    warnings.add(ValidationWarning.of(
                            ...
                            String.format("Group pattern only matches %d of %d sample files",
                ...
                if (result.getGroupsWithMissingRoles() > 0) {
                    warnings.add(ValidationWarning.of(
                            ...
                            String.format("%d groups are missing required image roles",
     ...
     */
    private List<ValidationError> combineErrors(List<ValidationError> list1, List<ValidationError> list2) {
     ...
     */
    private List<ValidationWarning> combineWarnings(List<ValidationWarning> list1, List<ValidationWarning> list2) {
