 ...
 * rules.
 * Implements precedence-based classification where overview rules exclude files
 * from front/rear consideration.
 */
public class RuleEngine {

    /**
     * Classifies a filename into an image role using the provided rules.
     * Rules are applied in precedence order (overview first, then front, then rear)
     * to ensure proper exclusion logic.
     * 
    ...
    public ImageRole classifyFilename(String filename, List<RoleRule> rules) {
        ...
        Map<ImageRole, List<RoleRule>> rulesByRole = rules.stream()
                .collect(Collectors.groupingBy(RoleRule::getTargetRole));
        ...
        switch (rule.getRuleType()) {
            ...
            case REGEX_OVERRIDE:
                try {
                    ...
                    throw new IllegalArgumentException("Invalid regex pattern in rule: " + rule.getRuleValue(), e);
                }
    ...
    /**
     * Generates a regex pattern for all rules targeting a specific image role.
     * The generated pattern will match any filename that would be classified as the
     * target role.
     * 
    ...
    public ValidationResult validateRules(List<RoleRule> rules) {
        ...
        if (rules == null) {
            errors.add(new ValidationError(ValidationErrorType.INVALID_RULE_CONFIGURATION,
                    "Rules list cannot be null"));
        ...
        for (int i = 0; i < rules.size(); i++) {
            ...
            if (rule.getRuleValue() == null || rule.getRuleValue().trim().isEmpty()) {
                warnings.add(new ValidationWarning(ValidationWarningType.EMPTY_RULE_VALUE,
            ...
            if (rule.getRuleType() == RuleType.REGEX_OVERRIDE) {
                try {
                    ...
                    errors.add(new ValidationError(ValidationErrorType.INVALID_REGEX_PATTERN,
        ...
        Set<ImageRole> rolesWithRules = rules.stream()
                .map(RoleRule::getTargetRole)
                .collect(Collectors.toSet());

        for (ImageRole role : ImageRole.values()) {
            if (!rolesWithRules.contains(role)) {
                warnings.add(new ValidationWarning(ValidationWarningType.MISSING_ROLE_RULES,
                        "No rules defined for " + role + " role - files will not be classified as " + role));
            }
        }
     ...
     */
    public Map<ImageRole, List<String>> classifyFilenames(List<String> filenames, List<RoleRule> rules) {
        if (filenames == null) {
