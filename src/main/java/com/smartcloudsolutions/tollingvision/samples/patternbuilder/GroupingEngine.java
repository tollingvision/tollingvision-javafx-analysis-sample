 ...
 * detection.
 * This ensures that grouping only uses the group ID token and doesn't include
 * camera side/role information.
 */
public class GroupingEngine {
    ...
    public static class GroupingResult {
        ...
        public GroupingResult(Map<String, List<String>> groups,
     ...
     */
    public GroupingResult groupAndAssignRoles(List<String> filenames,
            String groupPattern,
    ...
    /**
     * Assigns roles to files within each group using role rules.
     * Overview rules are applied first and exclude files from front/rear
     * consideration.
     */
    private void assignRolesWithinGroups(Map<String, List<String>> groups,
            List<RoleRule> roleRules,
            Map<String, ImageRole> fileToRole,
            List<String> unmatchedFiles,
            Map<String, String> unmatchedReasons) {
            ...
            case REGEX_OVERRIDE -> {
                try {
                    Pattern pattern = Pattern.compile(ruleValue,
                            rule.isCaseSensitive() ? 0 : Pattern.CASE_INSENSITIVE);
    ...
    public ValidationResult validateGroupPattern(String groupPattern) {
        if (groupPattern == null || groupPattern.trim().isEmpty()) {
            return ValidationResult.failure(
                    ValidationError.of(ValidationErrorType.EMPTY_GROUP_PATTERN));
        }
        ...
        try {
            ...
            if (capturingGroups == 0) {
                ...
                return ValidationResult.failure(
                        ValidationError.of(ValidationErrorType.MULTIPLE_CAPTURING_GROUPS,
            ...
            return ValidationResult.failure(
                    ValidationError.of(ValidationErrorType.REGEX_SYNTAX_ERROR,
                            "Invalid regex syntax: " + e.getMessage()));
        }
    }
