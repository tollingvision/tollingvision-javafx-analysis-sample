 ...
 * detection
 * and type suggestion based on common delimiters and pattern recognition.
 * Includes caching support for improved performance with large datasets.
 */
public class FilenameTokenizer {
    ...
    private static final Set<String> ALL_CAMERA_SYNONYMS = CAMERA_SYNONYMS.values()
            .stream()
            .flatMap(Set::stream)
    ...
    /**
     * Tokenizes a single filename using common delimiters.
     * Uses caching if available to improve performance.
     * 
    ...
    private List<FilenameToken> mergeDateTokens(List<FilenameToken> tokens) {
        ...
        for (int i = 0; i < merged.size(); i++) {
            ...
            merged.set(i, new FilenameToken(token.getValue(), i, token.getSuggestedType(), token.getConfidence()));
        }
     ...
     */
    public List<TokenSuggestion> suggestTokenTypes(Map<String, List<FilenameToken>> tokenizedFilenames) {
     ...
     */
    private Map<Integer, List<String>> groupTokensByPosition(Map<String, List<FilenameToken>> tokenizedFilenames) {
        ...
        for (List<FilenameToken> tokens : tokenizedFilenames.values()) {
            for (FilenameToken token : tokens) {
                tokensByPosition.computeIfAbsent(token.getPosition(), k -> new ArrayList<>())
                        .add(token.getValue());
            }
        }
     ...
     */
    private List<TokenSuggestion> analyzeTokenPosition(int position, List<String> values, int totalFiles) {
        List<TokenSuggestion> suggestions = new ArrayList<>();
        ...
        if (isLastPosition(position, values, totalFiles)) {
            ...
            if (extConfidence > 0.5) {
                suggestions.add(new TokenSuggestion(
                        TokenType.EXTENSION,
                        "File extension",
        ...
        if (uniquenessRatio > 0.7 && cameraConfidence < 0.3 && dateConfidence < 0.3 && indexConfidence < 0.3) {
            suggestions.add(new TokenSuggestion(
                    TokenType.GROUP_ID,
    ...
    private double analyzeExtensions(Set<String> values) {
        long extensionCount = values.stream()
                .mapToLong(v -> IMAGE_EXTENSIONS.contains(v.toLowerCase()) ? 1 : 0)
                .sum();
    ...
    private double analyzeDates(Set<String> values) {
        long dateCount = values.stream()
                .mapToLong(v -> DATE_PATTERNS.stream()
                        .anyMatch(pattern -> pattern.matcher(v).matches()) ? 1 : 0)
                .sum();
    ...
    private double analyzeIndexes(Set<String> values) {
        long indexCount = values.stream()
                .mapToLong(v -> INDEX_PATTERN.matcher(v).matches() ? 1 : 0)
                .sum();
     ...
     */
    private void applyTokenSuggestions(Map<String, List<FilenameToken>> tokenizedFilenames,
     ...
     */
    private TokenSuggestion findBestSuggestionForToken(FilenameToken token, List<TokenSuggestion> suggestions) {
        return suggestions.stream()
                .filter(s -> s.getExamples().contains(token.getValue()) ||
                        isTokenMatchingSuggestion(token, s))
    ...
    private boolean isTokenMatchingSuggestion(FilenameToken token, TokenSuggestion suggestion) {
        ...
        return switch (suggestion.getType()) {
            ...
            case DATE -> DATE_PATTERNS.stream()
