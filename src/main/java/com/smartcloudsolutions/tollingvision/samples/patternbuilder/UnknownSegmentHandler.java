...
/**
 * Handles unknown segments in filenames by allowing users to label them
 * as ignored, custom tokens, or free text to prevent files from falling
 * out of matching.
 */
public class UnknownSegmentHandler {
    ...
    public void labelSegment(String segmentValue, SegmentAction action, String customLabel) {
        segmentLabels.put(segmentValue.toLowerCase(),
                new SegmentLabel(segmentValue, action, customLabel));
    }
    ...
    public List<FilenameToken> applySegmentLabels(List<FilenameToken> tokens) {
            ...
            if (label != null) {
                switch (label.getAction()) {
                    ...
                    case CUSTOM_TOKEN -> {
                        ...
                        processed.add(new FilenameToken(
                                token.getValue(),
                                token.getPosition(),
        ...
        for (int i = 0; i < processed.size(); i++) {
            ...
            processed.set(i, new FilenameToken(
                    token.getValue(),
                    i,
                    token.getSuggestedType(),
                    token.getConfidence()));
        }
     ...
     */
    public UnknownSegmentSummary getUnknownSegmentSummary(Map<String, List<FilenameToken>> allTokens) {
    ...
    public static class UnknownSegmentSummary {
        ...
        public UnknownSegmentSummary(Set<String> allUnknownSegments,
