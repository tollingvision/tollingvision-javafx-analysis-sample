 ...
 * Provides summary information about how filenames are being categorized
...
public class PreviewSummary {
    ...
    public PreviewSummary(List<FilenamePreview> previews, java.util.ResourceBundle messages) {
        ...
        for (Map.Entry<String, Set<ImageRole>> entry : groupRoles.entrySet()) {
            ...
            // A group is considered incomplete if it has images but is missing front or rear
            // (overview is optional)
            if (!roles.isEmpty() && (!roles.contains(ImageRole.FRONT) || !roles.contains(ImageRole.REAR))) {
    ...
    /**
     * @return true if any warnings were generated (incomplete groups, unmatched files)
     */
    ...
    /**
     * @return true if the configuration appears to be working well (high match rate, few warnings)
     */
    public boolean isHealthy() {
        ...
        // Check incomplete groups ratio (allow up to 20% incomplete groups, but not exactly 20%)
    ...
    public String getSummaryText() {
        ...
        sb.append(String.format(messages.getString("preview.summary.files"),
                               totalFiles, matchedFiles, getMatchPercentage(), unmatchedFiles));
        ...
        if (!incompleteGroups.isEmpty()) {
            sb.append("\n").append(String.format(messages.getString("preview.role.missing"), incompleteGroups.size()));
        }
