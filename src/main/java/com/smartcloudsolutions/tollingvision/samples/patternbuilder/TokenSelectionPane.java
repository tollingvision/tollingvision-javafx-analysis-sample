...
/**
 * UI component for visual token display and drag-and-drop interaction.
 * Allows users to see detected tokens and their suggested types,
 * with the ability to modify token types through drag-and-drop or selection.
 */
public class TokenSelectionPane extends VBox {
    ...
    private void initializeComponents() {
        ...
        tokenDisplay.setStyle("-fx-background-color: #f8f9fa; -fx-border-color: #dee2e6; -fx-border-radius: 5;");

        tokenTypesPanel = new VBox(10);
        tokenTypesPanel.setPadding(new Insets(15));
        tokenTypesPanel.setStyle("-fx-background-color: #f8f9fa; -fx-border-color: #dee2e6; -fx-border-radius: 5;");
        ...
        explanationArea.setText(
                "Drag tokens to different type categories to adjust their classification. " +
                        "The system has automatically detected token types based on common patterns. " +
                        "You can override these suggestions by dragging tokens to the appropriate categories.");
    }
    ...
    private void createTokenTypeDropZone(TokenType tokenType) {
        ...
        dropZone.setStyle("-fx-background-color: #ffffff; -fx-border-color: #ced4da; " +
        ...
        typeLabel.setStyle("-fx-font-weight: bold; -fx-text-fill: " + getTokenTypeColor(tokenType) + ";");
        ...
        dropZone.setOnDragOver(event -> {
            if (event.getGestureSource() != dropZone && event.getDragboard().hasString()) {
                ...
                dropZone.setStyle("-fx-background-color: #e3f2fd; -fx-border-color: #2196f3; " +
                        "-fx-border-radius: 3; -fx-min-height: 60;");
            }
            event.consume();
        });
