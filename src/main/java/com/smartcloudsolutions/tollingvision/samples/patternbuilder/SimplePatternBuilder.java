 ...
 *
 * <p>
 * Task 15 Implementation: This class demonstrates live validation refresh and
 * empty-state
...
public class SimplePatternBuilder extends VBox {
     ...
     * live validation. Task 12: Integrates performance optimizations with caching
     * and background
     * processing.
     */
    ...
    /**
     * Sets up data bindings between components. Task 15: Implements live validation
     * refresh with
     * debounced updates.
     */
    private void setupBindings() {
        ...
        detectedTokens.addListener(
                (javafx.collections.ListChangeListener<FilenameToken>) c -> {
        ...
        roleRules.addListener(
                (javafx.collections.ListChangeListener<RoleRule>) c -> {
                    ValidationLogger.logConfigurationChange(
    ...
    /**
     * Sets up step navigation logic. Updated for new step numbering without File
     * Analysis step.
     */
    ...
    /**
     * Shows the specified step and updates navigation. Task 15: Implements
    ...
    private void showStep(int step) {
        ...
        switch (step) {
            case 0 -> {
                // Token Selection step - add reanalyze button, progress indicator, and token
                // preview
                // control
                ...
                getChildren()
                        .addAll(
                                getChildren().size() - 1, List.of(tokenHeader, previewControl, tokenSelectionPane));
            ...
            case 1 -> {
                ...
                updateTokenPreview(tokenPreviewCountSpinner.getValue());
                groupIdSelector.setAvailableTokens(detectedTokens);
        ...
        // Only run validation on the final step (step 2) and only if analysis is
        // complete
    ...
    private void showValidationDetails() {
        ...
        if (!validationBlocker.getActiveWarnings().isEmpty()) {
            ...
            TitledPane warningPane = new TitledPane(
    ...
    /**
     * Validates the current step to determine if user can proceed. Updated for new
     * step numbering:
     * 0=Token Selection, 1=Group ID, 2=Role Rules Early steps have lightweight
     * validation; only final
    ...
    private boolean validateCurrentStep() {
        return switch (currentStep) {
            ...
            case 1 ->
                selectedGroupId.get() != null; // Group ID Selection - group ID must be selected (no validation
                                               // blocking)
            case 2 ->
                analysisComplete
                        && !roleRules.isEmpty()
    ...
    /**
     * Starts file analysis for the given directory. Checks cache first, then
        ...
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    tokenPreviewCountSpinner.setValueFactory(
                            new SpinnerValueFactory.IntegerSpinnerValueFactory(1, maxPreview, 1));
    ...
    private void updateTokenPreview(int fileCount) {
        ...
        System.out.println("DETECTED TOKENS: " + detectedTokens);
    ...
    /**
     * Updates the validation state based on current configuration. Only runs
     * validation if analysis
     * is complete and we're on the final step.
     */
     ...
     * triggers debounced
     * validation refresh automatically. Only runs if analysis is complete and we're
     * on the final
     * step.
     */
    ...
    /**
     * Updates the validation status display in the UI. Task 15: Ensures consistent
     * validation display
    ...
    public PatternConfiguration generateConfiguration() {
        ...
        for (ImageRole role : ImageRole.values()) {
            List<RoleRule> rulesForRole = roleRules.stream().filter(rule -> rule.getTargetRole() == role).toList();
    ...
    /**
     * Task 15: Demonstrates that pattern generation succeeds when success banner is
     * shown.
     *
    ...
    /**
     * Clears the analysis cache for a specific directory. Should be called when the
     * input folder
    ...
    /**
     * Sets the directory for analysis and triggers file loading. Task 15: Triggers
     * validation refresh
     * when folder changes.
     *
    ...
    /**
     * Analyzes sample files from the specified directory. Task 15: Triggers
     * validation refresh when
     * files are analyzed.
     *
    ...
    /**
     * Cleans up resources when the pattern builder is no longer needed. Task 12:
     * Properly manages
