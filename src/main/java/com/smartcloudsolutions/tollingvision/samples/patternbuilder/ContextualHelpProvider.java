...
/**
 * Provides contextual help, tooltips, and explanatory text throughout
 * the pattern builder interface. Centralizes all help content for
...
public class ContextualHelpProvider {
        ...
        return switch (helpKey) {
            ...
            case "regex-patterns" ->
                "Regular expressions (regex) are patterns that match text. " +
                        "The pattern builder automatically generates regex from your visual configuration:\n" +
    ...
    public static String getTooltipText(String tooltipKey) {
        return switch (tooltipKey) {
            ...
            case "role-rules-optional" ->
                "Define at least one role rule to categorize images";

            case "preview-auto-update" ->
            ...
            case "pattern-explanation" ->
                "Click to see how this pattern works";

            case "sample-files" ->
                "First 500 files from the selected folder will be analyzed";

            case "token-confidence" ->
                "Confidence level for automatic token type detection";

            case "rule-priority" ->
                "Lower numbers have higher priority (overview rules always first)";

            case "preset-auto-save" ->
                "Configuration is automatically saved when patterns are applied";

            case "advanced-mode" ->
                "Edit regex patterns directly with live preview";

            case "simple-mode" ->
                "Visual pattern builder with automatic regex generation";
    ...
    private static Tooltip getOrCreateTooltip(String key) {
        return TOOLTIP_CACHE.computeIfAbsent(key, k -> {
